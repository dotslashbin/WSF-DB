...
-- NOTE: following tables must be loaded first: CurrencyConversion, Retailers, ForSaleDetail, StdBottleSize
-- =============================================
CREATE PROCEDURE [dbo].[__23UpdateImportPrices_01] 
	@IsUseOldWineN bit = 1
...
if @IsUseOldWineN = 1 begin
	with a as (
		...
		select a.wid, vinn = z.Wine_VinN_ID from RPOWine.dbo.Wine_N z join a on z.oldWineN = a.wineN 
	...
	with a as (
		...
		select a.wid, vinn = z.Wine_VinN_ID from RPOWine.dbo.Wine_N z join a on z.ID = a.wineN 
		group by wid, z.Wine_VinN_ID
	), c as (
		select wid, vinn from dbo.WAName where vinn > 0 
		group by wid, vinn
...
with a as (
	...
	from RPOWine.dbo.Wine_N group by case when @IsUseOldWineN = 1 then oldWineN else ID end
), d as (
...
--Deduce WineN
with a as (
	select vinn, vintage, wineN = case when @IsUseOldWineN = 1 then oldWineN else WineN end
	from RPOWine.dbo.Wine 
	where vinn is not null and vintage is not null and wineN is not null 
	group by vinn, vintage, case when @IsUseOldWineN = 1 then oldWineN else WineN end
), b as (
...
select 'newPrices',
     ...
     (select count(*) from dbo.ForSaleDetail where errors is not null),
     (select count(*) from dbo.ForSale where errors is not null),
     (select count(*) from dbo.Retailers),
     (select count(*) from dbo.Retailers where errors is not null),
     
     (select count(*) from RPOWine.dbo.Wine),
     (select count(*) from dbo.WAName),
     (select count(*) from dbo.WAName where errors is not null),
