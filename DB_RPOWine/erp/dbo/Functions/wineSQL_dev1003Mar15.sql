CREATE  function [dbo].wineSQL_dev1003Mar15 ( 
	 @Select varChar(1000)
	,@maxRows int = 100
	,@Where varchar(max) = ''
	,@GroupBy varchar(max) = ''
...
if isNull(@whN,0) <> 0 begin
	if 0<>charIndex(',cellarInfo,', @Select2) begin
		set @sm = 'case when bottleLocations is null then isNull(''['' + userComments + '']'', '''') when userComments is null then bottleLocations else bottleLocations +''
...
declare @s nvarchar(max) =  dbo.wineSQL_dev1003Mar15(
