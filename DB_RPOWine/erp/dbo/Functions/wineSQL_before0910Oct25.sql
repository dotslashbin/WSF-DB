CREATE  function [dbo].[wineSQL_before0910Oct25] ( 
	 @Select varChar(1000)
	,@maxRows int = 100
	,@Where varchar(max) = ''
	,@GroupBy varchar(max) = ''
...
/*
declare @s nvarchar(max) =  dbo.[wineSQL_after0910Oct24](
	' Select tastingN, wineN, taster, pubIconN, issue, producerShow, priceLo, publication, country, labelName, rating,notes',			--@Select2
	100			--@maxRows,
	--,'where vinn = 13718 and vintage like ''%177%'''		--@Where,
	--,'where vinn = 13718'						--@Where,
	,'where vinn = 942'						--@Where,
	,''								--@GroupBy,
