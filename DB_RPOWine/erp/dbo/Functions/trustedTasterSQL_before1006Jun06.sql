...
-----------------------------------------------------------------------------------------------------
CREATE  function [dbo].[trustedTasterSQL_before1006Jun06](@whN int, @contains nvarchar(1000)=null, @mustAlreadyBeTrustedByMe bit = 0, @maxRows int = 100)
returns varchar(max)
begin
...
select top '+convert(varchar, @maxRows)+' * from d '
...
if @mustAlreadyBeTrustedByMe <> 0
	...
	set @s = replace(@s, '@whereAlreadyTrusted@', '	where c.tasterN is null')	
