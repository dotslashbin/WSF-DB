...
CREATE function [dbo].[getPurchaseLocations_beforeApril112012] (@whN int, @supplierN int) 
returns @RR table (purchaseN int, bottleLocations varchar(max))
as begin
 
/*
select * from dbo.getPurchaseLocations (20, 8)
select * from dbo.getPurchaseLocations (20, null)
select * from purchase where whN=20
 
declare @whN int = 20
select name, purchaseDate, bottleLocations
	from purchase a
		join supplier b
			on a.supplierN = b.supplierN
		join dbo.getPurchaseLocations(@whN, null) c
			on a.purchaseN = c.purchaseN
		where a.whN = @whN
*/ 
