CREATE  function [dbo].wineSQL_dev1005May20b ( 
	 @Select varChar(1000)
	,@maxRows int = 100
	,@Where varchar(max) = ''
	,@GroupBy varchar(max) = ''
...
declare @s nvarchar(max) =  dbo.wineSQL_dev1005May20b(
	...
	--,'where contains ( encodedKeywords, ''  "pav*" and "2003*"'')  '				--@where
	--,'where ratingRange = ''96-100'' and contains ( encodedKeywords, ''  "pa*" and "2003*"'')  '				--@where
...
declare @s nvarchar(max) =  dbo.wineSQL_dev1005May20b(
	'Select rating'
	,100		--@maxRows,
	--,'where contains ( encodedKeywords, ''  "pavie*"   '')  and rating between 96 and 100  '				--@where
	,'where contains ( encodedKeywords, ''  "pavie*"   '')  '				--@where
