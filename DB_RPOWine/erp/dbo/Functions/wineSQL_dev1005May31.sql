CREATE  function [dbo].wineSQL_dev1005May31 ( 
	 @Select varChar(1000)
	,@maxRows int = 100
	,@Where varchar(max) = ''
	,@GroupBy varchar(max) = ''
...
set @sf +='
		'+case when @doGetAllTastings = 1 then '' else 'left ' end+'join vActiveTasting e
			on e.wineN = a.wineN
...
if 1 = @tm begin
            set @sf = @sf + '
		left join 
			(select distinct wineN from vActiveTasting) m
				on a.wineN = m.wineN'
end
...
if @pubGN is not null and @MustHaveReviewInThisPubG <> 0 
	begin
		...
		--else
		--	begin
		--		set @wherex = 'exists(select * from vActiveTasting where wineN=e.wineN and isErpTasting=1)'
...
declare @s nvarchar(max) =  dbo.wineSQL_dev1005May20b(
	'Select ratingRange'
	,100		--@maxRows,
	,'where contains ( encodedKeywords, ''  "pavie*"   '')  '				--@where
...
declare @s nvarchar(max) =  dbo.wineSQL_dev1005May20b(
	'Select rating'
	,100		--@maxRows,
	,'where contains ( encodedKeywords, ''  "pavie*"   '')     and rating between 80 and 89'				--@where
...
declare @s nvarchar(max) =  dbo.wineSQL_dev1005May31 (
     --'Select TastingN,wineN, vintage, fullWineName, colorClass, publication, pubiconN,  forSaleIconN, priceLo, estimatedCostLo, priceShow, priceShowRelease, ratingShow, maturity,MyIconN,MyBottleCount,ProducerURL, Site , Locale,Location,Region ,Country, variety, dryness,  winetype,ProducerShow,rating,hasUserComplaint,ratingMinor,cellarInfo,valuation'
     'Select TastingN,wineN, vintage, fullWineNamePlus, colorClass, publication, pubiconN,  forSaleIconN, priceLo, estimatedCostLo, priceShow, priceShowRelease, ratingShow, maturity,MyIconN,MyBottleCount,ProducerURL, Site , Locale,Location,Region ,Country, variety, dryness,  winetype,ProducerShow,rating,hasUserComplaint,ratingMinor,cellarInfo,valuation'
     ,1000          --@maxRows,
     --,'where contains ( encodedKeywords, ''  "pavie*" and "2003*"'')  '                 --@where
     --,'where contains ( encodedKeywords, ''"cabernet*"'')  and (hasUserComplaint=0 or (hasUserComplaint=1 and isApproved=1) or hasUserComplaint is null '                --@where
     ,'where contains ( encodedKeywords, ''"cabernet*"'')  and (hasUserComplaint=0 or (hasUserComplaint=1 and isApproved=1) or hasUserComplaint is null) '                --@where
...
declare  @s varchar(max); 
set @s = dbo.wineSQL_dev1005May31(
'Select country'--@Select,
,10--,@maxRows,
,'where contains ( encodedKeywords, ''  "cabernet*" '')  '--@Where,
,'Group by country'--@GroupBy,
,'Order by cnt desc'--@OrderBy,
,18223--@PubGN,
,0--@MustHaveReviewInThisPubG,
