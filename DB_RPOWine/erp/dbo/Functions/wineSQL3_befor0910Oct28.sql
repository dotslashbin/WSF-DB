CREATE  function [dbo].wineSQL3_befor0910Oct28 ( 
	 @Select varChar(1000)
	,@maxRows int = 100
	,@Where varchar(max) = ''
	,@GroupBy varchar(max) = ''
...
-- Fields needed
-------------------------------------------------------------------------------------
declare @select2 varchar(max), @ss varchar(5000)='', @sf varchar(5000)='', @sm varchar(5000)='', @sx varchar(max), @R varchar(5000)='', @allTastings bit=0, @Errors varchar(max)=null, @mustNotBeTrustedTaster bit = 0
...
-------------------------------------------------------------------------------------
 if isNull(@doGetAllTastings, 0) = 0 begin
...
--if isNull(@MustHaveReviewInThisPubG,0) = 0 and isNull(@mustBeTrustedTaster,0) = 0
if isNull(@MustHaveReviewInThisPubG,0) = 0
...
-- Trusted Tasters
-------------------------------------------------------------------------------------
if @whN is not null and isNull(@mustBeTrustedTaster,0) <> 0 
	begin
...
declare @s nvarchar(max) =  dbo.wineSQL3(
	...
	100			--@maxRows,
	--,'where vinn = 13718 and vintage like ''%177%'' and parkerZralyLevel>0'		--@Where,
	--,'where vinn = 13718 and vintage like ''%177%'''		--@Where,
	--,'where vintage like ''%177%'''		--@Where,
	--,'where vinn = 13718'						--@Where,
	--,'where vinn = 942  and isErpTasting=0'						--@Where,
	,'where wineN=84619 and parkerZralyLevel=0 and isErpTasting=0'					--@where
	,''								--@GroupBy,
