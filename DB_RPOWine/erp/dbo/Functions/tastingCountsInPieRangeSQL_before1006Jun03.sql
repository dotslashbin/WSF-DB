...
CREATE function [dbo].[tastingCountsInPieRangeSQL_before1006Jun03](@whN int = null, @wineN int = null, @pubGN int,  @pieOption int = 1)
...
declare 
	 @s varChar(max)
	,@where2 varchar(max) = ' and (hasUserComplaint=0 or (hasUserComplaint=1 and isApproved=1)) '
...
set @s =  dbo.[wineSQL_new](
	'Select @RC@@RR@, count(*) cnt'		--@Select
	,10000		--@maxRows,
