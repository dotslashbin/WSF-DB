CREATE  function [dbo].wineSQL_after1002Feb24 ( 
	 @Select varChar(1000)
	,@maxRows int = 100
	,@Where varchar(max) = ''
	,@GroupBy varchar(max) = ''
...
declare @s nvarchar(max) =  dbo.wineSQL_after1002Feb24(
	...
	'Select wineN, fullWineName, priceLo'			--@Select2
	,100		--@maxRows,
	--,'where contains ( encodedKeywords, ''  "pavie*" and "2003*"'')  '				--@where
	,'where isOfInterestX=1'--'where toBeDeliveredX = 1  '				--@where
	,''		--@GroupBy
	,''--'order by  fullWineName asc, Vintage desc'	--@OrderBy
	,9		--@PubGN
