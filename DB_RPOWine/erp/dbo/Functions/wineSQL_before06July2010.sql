...
CREATE  function [dbo].[wineSQL_before06July2010] ( 
	 @Select varChar(max)
	,@maxRows int = 100
	,@Where varchar(max) = ''
	,@GroupBy varchar(max) = ''
...
'
				...
				--1006Jun19     set @where += case when @where like '%where%' then ' and ' else 'where ' end + @wherex			
				if @wherex like '%[^ ]%'      set @where += case when @where like '%where%' then ' and ' else 'where ' end + @wherex			
...
if @doGetAllTastings=0 or @MustHaveReviewInThisPubG=1 or @mustBeTrustedTaster=1
	begin
	set @R = 'with' + dbo.tastingSQL(@whN, @pubGN, @doGetAllTastings, @MustHaveReviewInThisPubG, @mustBeTrustedTaster) + '
, a as (
