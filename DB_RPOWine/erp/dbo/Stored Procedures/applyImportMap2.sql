...
CREATE proc [dbo].[applyImportMap2](@whn int, @userWinesId int) as begin
...
--	, @commentsOtherVintage nvarchar(max)='<b><small>(User Added Vintage)</small></b>'
--	, @commentsOtherHistory nvarchar(max)='<b><small>(Other Users Matched This)</small></b>'
--	, @commentsSelected nvarchar(max)=' <b><small>(The Selected Wine) <==</small></b> '
--	, @commentsSelectedNew nvarchar(max)='<b><small>(The Selected Wine) <==<br>(New Vintage)</small></b> '
-----------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------
--1107Jul15     exec dbo.addUserWinesFields @myTableName , 'drop view vUserWines; drop view vu'
...
--						on c.wineN=b.wineN
			...
			update a
				...
				--from (select * from vUserWines where myWineName like @myWineName) a
				from vUserWines a
					join (select min(winen) winen, min(erpwinename)erpwinename, myvintage,mywinename from importmap d
