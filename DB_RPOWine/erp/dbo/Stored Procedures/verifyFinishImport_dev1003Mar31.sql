...
CREATE  proc verifyFinishImport_dev1003Mar31(@whN int, @finish bit) as
begin
...
--begin try
	...
	declare @minSmallDateTime smallDateTime = convert(smallDateTime,0), @today date = getDate(), @original varchar(max), @sizeMultiplier int = 1000, @i int
			...
			, @groupVinn int = null, @groupWineN int = null, @groupErpWineName varchar(200) = null
			, @count int
	...
	while 1=1
		...
		if @finish=0
			...
			------------------------------------------------------------------------------------------		
			select @wineN = null
			if @errors is null and (1=isNumeric(@wineNFromMatch) or @groupWineN is not null)
				begin
						...
						begin
							if @groupWineN is not null
								begin
									...
									if @count = 1
										begin
											set @sql = 'update transfer..userwines'+ convert(nvarchar, @whN) + N' set wineN = @wineN, erpWineName = @erpWineNme where id = @id'
			...
			-- on the first row of each group, remember the wineN and vinn
			if 1 = isNumeric(@wineNFromMatch) and (@priorMyWineName is null or @priorMyWineName <> @myWineName)
				begin
					select @groupWineN = @wineN, @groupErpWineName = @erpWineName
