CREATE proc summarizeBottleValuation_after1003Mar03 (@whN int, @wineN int = null, @priceGN int = null) as
--ALTER proc summarizeBottleValuation_after1003Mar03 (@whN int, @wineN int = null, @priceGN int = null) as
begin
...
,g as (select c.wineN, c.effectiveBottles
	...
	,  isNull(       isNull(e.mostRecentPriceHi, e.mostRecentPriceLo)              , isNull(pp.priceFromPurchase,         isNull(f.estimatedCostHi,f.estimatedCostLo)             )) pricePerBottleHi
	from c
		join wine f
			on f.wineN = c.wineN
		left join price e
