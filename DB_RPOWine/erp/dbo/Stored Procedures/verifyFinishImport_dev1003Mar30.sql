...
create  proc verifyFinishImport_dev1003Mar30(@whN int, @finish bit) as
begin
				...
				if @errors is null
					begin
						...
						if @wineN is null
							...
							begin
								if @myWineName <> @priorMyWineName set @repCount = 0
								select
									  @erpWineName = dbo.getFullWineNamePlus(@wineN)
									, @sql = 'update transfer..userwines'+ convert(nvarchar, @whN) + N' set rowFlag = @flagRemembered, wineN = @wineN, erpWineName = @erpWineName where id = @id'
								exec sp_executeSQL @sql, N'@id int, @flagRemembered int, @wineN int, @erpWineName varchar(200)',  @id, @flagRemembered, @wineN, @erpWineName
							end						
					end
				else
					begin
...
/*if @finish = 0
	begin
		...
		------------------------------------------------------------------------------------------	
		with a as (select row_number() over(order by id) ii, * from transfer..userwines20) update a set a.rowFlag = b.rowFlag from a 	join a b on a.ii = b.ii-1 and a.myWineName = b.myWineName and a.rowFlag is null and b.rowFlag <> @flagRemembered
