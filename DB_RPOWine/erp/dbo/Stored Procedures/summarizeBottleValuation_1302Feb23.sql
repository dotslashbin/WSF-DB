/*
summarizeBottleValuation_dev1003Mar09 20, null, null
 
use erp
summarizeBottleValuation_dev1003Mar09 19, null, null
summarizeBottleValuation_dev1003Mar09 19, 69797, null
select valuation from whToWine where whN=19 
 
select * from price where wineN=26404
*/
 
CREATE proc summarizeBottleValuation_1302Feb23 (@whN int, @wineN int = null, @priceGN int = null) as
--ALTER proc summarizeBottleValuation_dev1003Mar09 (@whN int, @wineN int = null, @priceGN int = null) as
begin
...
update j
	...
	from whToWine j
		join h
			on j.wineN = h.wineN
	where j.whN = @whN 
		and ((@wineN is null or j.wineN = @wineN) )
		--and isNull(j.valuation,-1) <> isNull(h.valuation, -1);
		and 
		(
			j.valuation <> h.valuation
			or not(j.valuation is null and h.valuation is null)
		)
