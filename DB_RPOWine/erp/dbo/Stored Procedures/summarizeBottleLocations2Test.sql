CREATE proc summarizeBottleLocations2Test (@whN int, @wineN int) as
begin
...
-----------------------------------------------------------------------------------
/*
summarizeBottleLocations2Test 20, null
...
select wineN, bottleLocations from whToWine where whN=20 and bottleLocations like '%22%'
select wineN, bottleLocations from whToWine where whN=20 and bottleLocations like '% to %'
 
ERROR LAST TWO ITEMS ARE DUPLICATE
9 Bottles in Atlanta Left Wall Top Col 3, Col-5c to Col-5c
 
ERROR USING "-" INSTEAD OF COMMA
224 Bottles in Atlanta Right Wall, Florida - New York
...
select wineN, dbo.concatFF(convert(varchar, ii)+char(12)+path)locs
	from g
	where wineN in (91536,92523)
	group by wineN, litres
	order by wineN, litres
/*	

,h as (select top 199999 wineN, litres, max(nameInSummary) nameInSummary, sum(bottleCount) bottleCount, dbo.compactLocationSummary2(dbo.concatFF(convert(varchar, ii)+char(12)+path))locs 
	from g
	group by wineN, litres
	order by wineN, litres
	)
,j as (select top 199999 wineN, (convert(varchar, bottleCount) + ' ' + nameInSummary + case when bottleCount = 1 then '' else 's' end + ' in ' + locs) botSum 
	from h litres order by wineN, litres desc)
,k as (select wineN, replace(dbo.concatFF(botSum),char(12), ';  ') bottleLocations from j group by wineN)
update m
