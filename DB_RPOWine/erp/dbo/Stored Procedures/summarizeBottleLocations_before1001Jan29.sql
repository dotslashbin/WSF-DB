CREATE proc summarizeBottleLocations_before1001Jan29 (@whN int, @wineN int) as
begin
...
,b as (select wineN, litres, max(nameInSummary) nameInSummary
		...
		, sum (case when deliveryDate is null or deliveryDate <= '1900' or deliveryDate > @today then bottleCountBought else 0 end) notYetDeliveredBottleCount
		, sum (case when deliveryDate is null or deliveryDate <= '1900' or deliveryDate > @today then 0 else isNull(bottleCount, bottleCountBought) end) remainingBottleCount
	from a
