-- myWine management function xx [=]
CREATE proc forceUpdateMyWineOverview(
	 @whN int
	,@wineN int
	,@tastingCount int
	,@bottleCount int
...
declare @locationN int
 
set @location = replace(@location, '   ', ' ')
set @location = ltrim(rtrim(replace(@location, '  ', ' ')))
 
if @location is not null begin
	select @locationN = locationN from bottleLocation where whN = @whN and location = @location
	if @locationN is null begin
		insert into bottleLocation(whN, location) select @whn, @location
		set @locationN = @@identity
		end
	end
 
if exists(select 1 from whToWine where whN = @whN and wineN = @wineN) begin
	update whToWine
		set bottleCount = @bottleCount
			,tastingCount = isNull(@tastingCount, tastingCount)
			,wantToSellCount = isNull(@wantToSellCount, wantToSellCount)
			,wantToBuyCount = isNull(@wantToBuyCount, wantToBuyCount)
	...
	insert into whToWine(whN,wineN,bottleCount,tastingCount,wantToSellCount,wantToBuyCount,buyerCount,sellerCount,isOfInterest, locationN)
		select @whn, @wineN,isNull(@bottleCount,0),isnull(@tastingCount,0),isnull(@wantToSellCount,0),isnull(@wantToBuyCount,0)
			,isnull(@buyerCount,0),isnull(@sellerCount,0),@isOfInterest,@locationN
	end
...
/*
create function getMyWineInfoOverview (@whn int, @wineN int)
as begin
returns @T table(
	 whN int
	,wineN int
...
addBottles(whN, wineN, bottleCount, location)
addTasting(whN, wineN, rating, maturity, notes)
updateWantToSell(whN, wineN, wantToSellCount, minPriceWanted, buyerCount, fromLocation)
updateWantToBuy(whN, wineN, wantToBuyCount, maxPriceToPay, sellerCount)
