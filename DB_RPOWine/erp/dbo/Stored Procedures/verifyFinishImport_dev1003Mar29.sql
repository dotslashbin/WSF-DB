...
CREATE  proc verifyFinishImport_dev1003Mar29(@whN int, @finish bit) as
begin
...
--begin try
	...
	declare @minSmallDateTime smallDateTime = convert(smallDateTime,0), @today date = getDate(), @original varchar(max), @sizeMultiplier int = 1000, @i int
			...
			, @rowflag int = 0, @flag1 int = 1, @flag2 int = 2
			, @repCount int = 0
							...
							begin
								if @myWineName = @priorMyWineName
									begin
										...
										exec sp_executeSQL @sql, N'@mp cursor, @rowFlag int',  @mp, @rowFlag
									end
								else
									set @repCount = 0
							end
