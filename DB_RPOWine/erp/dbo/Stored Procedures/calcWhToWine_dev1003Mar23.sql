CREATE proc [dbo].calcWhToWine_dev1003Mar23(@whN int, @wineN int, @fullRecalculateFromMyWinesTables bit = 0)
as begin
set noCount on;
...
update whToWine
			...
			, notYetCellaredShow = case when isNull(notYetCellaredBottleCount, 0) <> 0 then 1 else 0 end
                        ...
                        , hasBottlesX = case when isNull(hasBottlesShow, 0) <> 0 or isNull(hasBottlesR, 0) <> 0          then 1 else 0 end
