/*
oofrr
calcWhToWine_after1001Jan25 20,null
select wineN, toBeDeliveredBottleCount, toBeDeliveredX, notYetCellaredBottleCount, notYetCellaredX, bottleCount,tastingCount,wantToSellBottleCount,wantToBuyBottleCount,buyerCount,sellerCount,purchaseCount 
	from  whtowine where whn=20
 
 
os whtowine, 'where whN=20', 1
select * from whToWine where whn=20
*/
CREATE proc [dbo].calcWhToWine_after1003Mar04(@whN int, @wineN int, @fullRecalculateFromMyWinesTables bit = 0)
as begin
set noCount on;
if @fullRecalculateFromMyWinesTables <> 0
            --exec dbo.calcMyWinesTotals @whN, @wineN
            --KLUDGE
            exec dbo.calcMyWinesTotals @whN, null
else 
            begin
