CREATE proc [dbo].verifyFinishImportTry_1201Jan22(@whN int, @finish bit) as
begin
	...
	begin
		if @finish=1
			begin
				set @sql= replace(
					';with
					t as	(select * from transfer..userwines@@     )
					,a as	(select mywinename, vintage, min(wineN) winen from t where isdetail=0 and wineN>0 group by mywinename, vintage     )
					,b as	(select * from t where isDetail=1)
					update b set b.wineN=a.wineN
						from b 
							join a on a.mywinename=b.mywinename and a.vintage=b.vintage'
					, '@@'
					, @whN     )
...
---------------------------------------------------------------------------------- 
	...
	--update vUserWines set myWineName=replace(replace(replace(myWineName,'''''''''''',''''),'''''''',''''),'''','''')
	set @sql = 'update transfer..userWines@whN set myWineName=replace(replace(replace(myWineName,'''''''''''''''''''''''',''''''''),'''''''''''''''',''''''''),'''''''','''''''')'
	set @sql = replace (@sql,'@whN',convert(nvarchar,@whN))
	exec (@sql)
		...
		begin
			set @sql= replace(
				'update transfer..userWines@@ set 
					...
					,  PurchaseQuantity = isNull(dbo.nullNormalizeName(PurchaseQuantity),1)
		...
		begin
			...
			--if @purchaseQuantity like '%[123456789]%'
				--begin
					...
					begin try set @bottleCount = isNull(convert(int, @PurchaseQuantity),1) end try begin catch set @bottleCount = null end catch
			...
			if @deliveryDateRaw is null or @deliveryDateRaw  not like '%[0-z]%'
				begin
					-----------------------------------------------------------------------------------
							...
							update whToWine set 
								  userComments =	case when @location like '%[^ ]%' 
													 then	case when userComments like '%[^ ]'
																then case when len(userComments+'; '+@location)>@userCommentsSize
																		then left(userComments+'; '+@location,(@userCommentsSize-4))+' ...'
																		else userComments+'; '+@location
																		end
																else case when len(@location)>@userCommentsSize
																		then left(@location,(@userCommentsSize-4))+' ...'
																		else @location
																		end
															end
													 else userComments 
