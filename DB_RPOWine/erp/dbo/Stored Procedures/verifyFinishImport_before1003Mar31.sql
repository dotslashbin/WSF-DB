--use erp
CREATE proc verifyFinishImport_before1003Mar31(@whN int, @finish bit) as
--ALTER  proc verifyFinishImport_dev1003Mar28(@whN int, @finish bit) as
begin
--declare @whN int = 20, @finish bit = 0
set noCount on
declare  @importStatus int = 0
--begin try
	declare	@vintage varchar(10), @myWineName varchar(200), @erpWineName varchar(200), @Supplier varchar(max), @PurchaseQuantity varchar(max), @BottleSize	varchar(max), @PurchaseDate datetime, @deliveryDate datetime, @Price varchar(max), @Location varchar(max), @wineNFromMatch int, @myVintage varchar(max)
	...
	while 1=1
		...
		--if @errors like '<BR>%' select @errors = replace(substring(@errors, 5, 99999), '<BR>', '<BR><HR><BR>'), @importStatus=1 else set @errors = null
		if @errors like '<BR>%' select @errors = replace(substring(@errors, 5, 99999), '<BR>', '<HR>'), @importStatus=1 else set @errors = null
		------------------------------------------------------------------------------------------
			...
			begin			
				if @errors is null
					begin
						...
						set @wineN = null
						select @wineN = wineN from whNameToWine where whN=@whN and myWineName = @myWineName and vintage = @vintage and isOld=0
						if @wineN is not null
							begin
								
								select
									@erpWineName = dbo.getFullWineNamePlus(@wineN)
									, @sql = 'update transfer..userwines'+ convert(nvarchar, @whN) + N' set rowFlag = 4, wineN = @wineN, erpWineName = @erpWineName where current of @mp'
								exec sp_executeSQL @sql, N'@mp cursor, @wineN int, @erpWineName varchar(200)',  @mp, @wineN, @erpWineName
							end						
					end
				else
					begin
...
verifyFinishImport_dev1003Mar28 20, 0
verifyFinishImport_dev1003Mar28 20, 1     
