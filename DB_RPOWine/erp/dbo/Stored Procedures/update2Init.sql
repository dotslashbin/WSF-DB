...
select * into erpIn..jWine from erpsearchd..wine
select * into erpIn..eWine from rpowinedatad..wine					
*/
--drop synonym vTastingN
--begin try exec ('create synonym vTastingN for erpIn..tastingNew') end try begin catch end catch
 
begin try drop view veWine end try begin catch end catch;
exec ('create view veWine as select *  from  erpIn..eWine')
 
begin try drop view vjWine end try begin catch end catch;
exec ('create view vjWine as select * from (select row_number() over (partition by winen  order by fixedid )ii, * from  erpIn..jWine)a where ii=1')
 
begin try drop view vTastingNew end try begin catch end catch;
exec ('create view vTastingNew as select * from erpIn..tastingNew')
...
alter Index all on erpIn.dbo.jWine disable;
alter index ix_jwine_wineN on erpin..jwine rebuild
...
alter Index all on erpIn.dbo.jWine rebuild;
 
exec dbo.ooLog '     jWine JoinX added'
 
alter Index ix_eWine_joinx on erpIn.dbo.eWine disable;
...
alter Index ix_eWine_joinx on erpIn.dbo.eWine rebuild;
