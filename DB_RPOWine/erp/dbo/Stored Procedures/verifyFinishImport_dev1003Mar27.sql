...
verifyFinishImport_dev1003Mar27 20, 0
verifyFinishImport_dev1003Mar27 20, 1     
...
CREATE  proc verifyFinishImport_dev1003Mar27(@whN int, @finish bit) as
begin
--declare @whN int = 22, @finish bit = 0
set noCount on
			...
			--if not exists (select * from whNameToWine where whN=@whN and myWineName = @myWineName and vintage = @vintage and wineN = @wineN)
				...
				if @errors is null
					begin
						...
						set @sql = 'update transfer..userwines'+ convert(nvarchar, @whN) + N' set rowFlag = '+convert(varchar, @price%4)+' where current of @mp'
						exec sp_executeSQL @sql, N'@mp cursor, @err',  @mp
					end
