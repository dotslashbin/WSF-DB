CREATE proc summarizeBottleLocations_after1001Jan23 (@whN int, @wineN int) as
begin
...
-----------------------------------------------------------------------------------
/*
summarizeBottleLocations_after1001Jan23 20, null
...
-----------------------------------------------------------------------------------
with
g as (select top 199999 b.wineN, d.nameInSummary, d.litres
		, a.bottleCount purchaseBottleCount
		, case when deliveryDate is null or deliveryDate < purchaseDate then a.bottleCount else 0 end toBeDeliveredCount
		, e.bottleCountHere bottleCount, f.path, f.iiPath, f.depth, f.ii
...
,h as (select top 199999 wineN, litres, max(nameInSummary) nameInSummary
		, sum(purchaseBottleCount) purchaseBottleCount, sum(toBeDeliveredCount) toBeDeliveredCount
		, sum(bottleCount) bottleCount, dbo.compactLocationSummary(dbo.concatFF(convert(varchar, ii)+char(12)+path))locs 
	from g
...
,j as (select top 199999 wineN, (convert(varchar, bottleCount) + ' ' + nameInSummary + case when bottleCount = 1 then '' else 's' end + ' in ' + locs) botSum 
	from h order by wineN, litres desc)
,k as (select wineN, RTrim(replace(dbo.concatFF(botSum),char(12), '
