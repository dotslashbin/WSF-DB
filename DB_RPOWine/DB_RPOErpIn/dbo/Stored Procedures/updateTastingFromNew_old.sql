CREATE proc [updateTastingFromNew_old]
/*
use erpIn
 
alter view vTasting as select * from erp14..tasting
*/
as begin
...
print 'DataSourceN updated: '+convert(char, @@rowCount)
...
insert into @TSources(dataSourceN)
	select distinct(dataSourceN) from tastingNew;
...
------------------------------------------------------------------------------------------
-- BUG - have to limit to same datasource
------------------------------------------------------------------------------------------ 
