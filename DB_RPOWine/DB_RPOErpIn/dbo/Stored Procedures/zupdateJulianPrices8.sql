--use erpin
CREATE proc [zupdateJulianPrices8] as begin
...
begin try drop table nameYearRaw2c1 end try begin catch end catch
/*
oon nameYearRaw2
*/
...
--1.a     set vinn2 to the eRP vinn for the most recent vintage for this wid
...
,d as	(select wid, vinn
 ...
 --1.a     set producer2 and producerShow2 to erp producer when there is a vinn
...
--1.a     translate other wines with same wid producer
...
--1.a     fill in the remaining ones from vAlertOK
update vAlertOK set producer2 = replace(prod, '"', ''),producerShow2 = replace(prodShow, '"', '') where producer2 is null
 
 
 
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--1ab.     Create new non-erp Vinns for wid mapping
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--1ab1.		make sure we have all the vinns already in use from erpWine and vAlert
...
merge allocateWidVinn as aa
	using	(select max(wid)wid, vinn from vAlertOK where vinn is not null and errors is null group by vinn) as bb
on aa.vinn = bb.vinn
when matched then
...
--1ab2.     create vinn's from 1 milllion down.  reserve above 1 million for created wineN for existing vinn
...
---declare @date smallDateTime = getDate()
--2.1     Add in all current eRP wines
...
--2.1a     Update Vinn2 from allocateVinn
...
--2a4.    add in vAlertOK for forSale items with no vinn.  use newly created vinn then create a wineN
--2a5     add in erpFields for vAlertOK not already in nameyear

begin try drop table fforSale1 end try begin catch end catch
begin try drop table fforSale2 end try begin catch end catch
begin try drop table fforSale3 end try begin catch end catch
begin try drop table fforSale4 end try begin catch end catch
begin try drop table fforSale5 end try begin catch end catch

--2a1.    add in erpFields for forSale items with wineN	

select wid, wineN, vintage into fforSale1 from vForSaleOK where errors is null group by wid, wineN, vintage;
...
insert into nameYearRaw(phase, wineN, vinn, wid, producer, producerShow, labelName,vintage,country, region, location, variety, colorClass, dryness, wineType,dateCreated, dateUpdated)
	select '2a1', a.wineN, b.vinn,  a.wid, producer, producerShow, labelName, b.vintage, country, region, location, variety, colorClass, dryness, wineType, null, @date
	from fforSale1 a
		join b 
			on b.wineN = a.wineN and b.ii=1;
			
select * into nameYearRaw2c1 from nameYearRaw;
--end		--temp1


 ---declare @date smallDateTime = getDate();
--2a2.     add in erp wine where wineN can be deduced from erpWine through wid=>vinn, vintage     ACTIVE
begin try drop table fforSale2 end try begin catch end catch;
select a.wid, a.vintage, null wineN2 into fforSale2
	from (select wid, vintage from fforSale1 group by wid, vintage) a
...
insert into nameYearRaw(phase, wineN, vinn, wid, producer, producerShow, labelName,vintage,country, region, location, variety, colorClass, dryness, wineType,dateCreated, dateUpdated)
	select '2a2', g.wineN, g.vinn,  g.wid, producer, producerShow, labelName, g.vintage, country, region, location, variety, colorClass, dryness, wineType, null, @date
	from g
		join h
			on g.wineN = h.wineN and h.ii = 1
---     (647 row(s) affected)

select * into nameYearRaw2c2 from nameYearRaw;
begin try drop table fforSale3 end try begin catch end catch;
select a.wid, a.vintage, null wineN2 into fforSale3
	from (select wid, vintage from fforSale2 group by wid, vintage) a
		left join (select wid, vintage from nameYearRaw group by wid, vintage) b
			on a.wid=b.wid and a.vintage = b.vintage
	where b.wid is null and b.vintage is null;
...
--end		--temp2
/*
select phase, count(*)cnt from nameYearRaw group by phase
select count(*) from fforSale1
select count(*) from fforSale2
select count(*) from fforSale3

oon xx

sp_rename mapWidVinn, XXmapWidVinn
sp_rename mapWidWine, XXmapWidWine
sp_rename mapWineVinn, XXmapWineVinn
...
--2a4.    add in vAlertOK for forSale items with no vinn.  use newly created vinn then create a wineN
--2a5     add in erpFields for vAlertOK not already in nameyear
--2a6     ?? add in wine alert db not currently for sale
