--drop proc updateJulianPrices3

CREATE proc [zupdateJulianPrices3] as 
begin
...
with 
 a as (select wid, vinn2 from vAlert where vinn2 is null) 
,b as (select wid, wineN, row_number() over(partition by wid order by vintage desc, id)ii from vForSale where wineN is not null) 
