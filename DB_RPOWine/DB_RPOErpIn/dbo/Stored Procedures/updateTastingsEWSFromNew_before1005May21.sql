--use erpin
CREATE proc updateTastingsEWSFromNew_before1005May21
as begin
...
updateTastingsEWSFromNew
	(1134 row(s) affected)
	(8 row(s) affected)
	
select top 10 rowVersion, pubN, pubDate, wineN	from tasting order by rowversion desc
 
select pubN, pubDate from tastingNew where wineN=93792
select * from tasting where pubN is null and pubDate is null and wineN=93792
select * from tastingNew where pubN is null and pubDate is null and wineN=93792
select count(*) from tasting
	173648
updateTastingsEWSFromNew
select count(*) from tasting
	...
	--print @ewsPubN
	--select count(*) from tasting where pubN= 7024
	--select count(*) from tastingNew where pubN= 7024
...
*/	
	...
	--merge tasting a
with
--a as (select * from tasting where pubN=@ewsPubN and pubDate is not null and wineN is not null and wineN = 93792)
a as (select * from tasting where pubN=@ewsPubN and pubDate is not null and wineN is not null)
--, b as (select * from tastingNew where pubN=@ewsPubN and pubDate is not null and wineN is not null and wineN = 93792) 
, b as (select * from tastingNew where pubN=@ewsPubN and pubDate is not null and wineN is not null) 
	merge a
		using b
			on a.pubN = b.pubN and a.pubDate = b.pubDate and a.wineN = b.wineN
	when matched 
			and	(     
