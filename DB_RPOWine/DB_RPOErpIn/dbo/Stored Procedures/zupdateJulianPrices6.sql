Create proc [zupdateJulianPrices6] as begin
...
begin try drop table #forSale1 end try begin catch end catch
begin try drop table #forSale2 end try begin catch end catch
begin try drop table #forSale3 end try begin catch end catch
begin try drop table #forSale4 end try begin catch end catch
begin try drop table #forSale5 end try begin catch end catch
---select count(*) from #forSale1
 
---drop table #forSale1;drop table #forSale2;drop table #forSale3;drop table #forSale4;drop table #forSale5

--2a1.    add in erpFields for forSale items with wineN	
select wid, wineN, vintage into #forSale1 from vForSaleOK where errors is null group by wid, wineN, vintage;
...
---- => (28430 row(s) affected)
 
 
 
--2a2.     add in erp wine where wineN can be deduced from erpWine through wid=>vinn, vintage     NEEDS RETHINKING
...
----select * from #forSale;	--@debugging
----select * from #forSale2;	--@debugging
----select count(*) from #forSale2 => 44272
----select * from vAlertOK
----oodef vAlertOK
