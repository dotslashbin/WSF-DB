--use erpin
CREATE proc [zupdateJulianPrices11] as begin
...
update a set a.phase = b.phase
  ...
  -----------------------------------------------------------------------------------------------------------------------------------------
  --WHY SOME WID LEFT IN FFORSALE
  -----------------------------------------------------------------------------------------------------------------------------------------
  
  select distinct(wid) from fforsale where phase is null
  select count(distinct(wid)) from fforsale where phase is null
  
  select * from vAlertOK where wid in ('al1100123', 'al1101112', 'al1101113', 'al1101118', 'al1103110')
...
--bbdb>.    add in vAlertOK for forSale items with no vinn.  but there is a no valid erp producer.   use newly created vinn then create a wineN
--bbe>     add in erpFields for vAlertOK not already in nameyear
--bbf>     ?? add in wine alert db not currently for sale							   h
