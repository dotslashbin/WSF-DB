-- =============================================
-- Author:		Alex B.
-- Create date: 5/22/14 / 10/23/14
-- Description:	Processes data from RPOSearch database:
...
-------- WineTypes
--select WineType, erpWineType, count(*) from dbo.SYN_t_WAName group by WineType, erpWineType
insert into WineType (Name, WF_StatusID)
select isnull(wn.erpWineType, wn.WineType), WF_StatusID = 100
from dbo.SYN_t_WAName wn
	join dbo.SYN_t_ForSaleDetail sd on sd.Wid = wn.Wid
...
where wn.Wine_VinN_ID is NULL and isnull(sd.DollarsPer750Bottle, 0) > 0
	and isnull(wn.erpWineType, wn.WineType) is not null
	and isnull(isnull(wn.erpWineType, wn.WineType),'') != '' 
	and c.ID is NULL
...
from dbo.SYN_t_WAName wn
	join dbo.SYN_t_ForSaleDetail sd on sd.Wid = wn.Wid
	left join WineLabel l on isnull(wn.erpLabelName, wn.LabelName) = l.Name
where wn.Wine_VinN_ID is NULL and isnull(sd.DollarsPer750Bottle, 0) > 0
	and isnull(wn.erpLabelName, wn.LabelName) is not null
	and isnull(isnull(wn.erpLabelName, wn.LabelName),'') != ''
	and l.ID is NULL
...
where wn.Wine_VinN_ID is NULL and isnull(sd.DollarsPer750Bottle, 0) > 0
	and isnull(wn.erpProducer,wn.Producer) is not null
	and isnull(isnull(wn.erpProducer,wn.Producer),'') != ''
	and l.ID is NULL
...
where wn.Wine_VinN_ID is NULL and isnull(sd.DollarsPer750Bottle, 0) > 0
	and wn.Dryness is not null
	and isnull(wn.Dryness, '') != '' 
	and l.ID is NULL
group by wn.Dryness 
...
where wn.Wine_VinN_ID is NULL and isnull(sd.DollarsPer750Bottle, 0) > 0
	and wn.ColorClass is not null
	and isnull(wn.ColorClass, '') != ''
	and l.ID is NULL
...
where wn.Wine_VinN_ID is NULL and isnull(sd.DollarsPer750Bottle, 0) > 0
	and wn.Variety is not null
	and isnull(wn.Variety,'') != ''
	and l.ID is NULL
group by wn.Variety  
...
where wn.Wine_VinN_ID is NULL and isnull(sd.DollarsPer750Bottle, 0) > 0
	and sd.Vintage is not null
	and isnull(sd.Vintage,'') != ''
	and l.ID is NULL
group by sd.Vintage 
...
select
	...
	LabelID = isnull(wl.ID, 0), LabelName = max(isnull(isnull(wan.erpLabelName, wan.LabelName), '')),
...
into #t
from dbo.SYN_t_WAName wan
	join dbo.SYN_t_ForSaleDetail sd on sd.Wid = wan.Wid
	...
	left join WineLabel wl on isnull(isnull(wan.erpLabelName, wan.LabelName), '') = wl.Name
	left join WineVariety wv on isnull(wan.erpVariety, '') = wv.Name
	left join WineDryness wd on isnull(wan.erpDryness, '') = wd.Name
	left join WineColor wc on isnull(wan.erpColorClass, '') = wc.Name
	left join WineVintage wvin on isnull(sd.Vintage, '') = wvin.Name
	
	left join LocationCountry lc on isnull(wan.erpCountry, '') = lc.Name
	left join LocationRegion lr on isnull(wan.erpRegion, '') = lr.Name
	left join LocationLocation ll on isnull(wan.erpLocation, '') = ll.Name
	--left join LocationLocale lloc on isnull(wan.Locale, '') = lloc.Name
	--left join LocationSite ls on isnull(wan.Site, '') = ls.Name
where --wan.Wine_VinN_ID is NULL	-- everything, because there are could be Wines with existing VinN
	isnull(sd.DollarsPer750Bottle, 0) > 0		-- only stuff with prices
	and len(isnull(sd.Errors, '')) = 0
group by 
...
begin tran
	...
	from #t
		left join Wine_VinN vin on #t.ProducerID = vin.ProducerID
			...
			and #t.locLocationID = vin.locLocationID 
			and (#t.VinN = vin.ID or (vin.locLocaleID = 0 and vin.locSiteID = 0))
	...
	print 'Wine_VinN: ' + cast(@@rowcount as varchar(30)) + ' row(s) added.'
...
begin tran
	...
	from #t
		join Wine_VinN v on #t.ProducerID = v.ProducerID and #t.TypeID = v.TypeID 
			...
			and (#t.VinN = v.ID or (v.locLocaleID = 0 and v.locSiteID = 0))
	...
	print 'Wine_N: ' + cast(@@rowcount as varchar(30)) + ' row(s) added.'
	
commit tran

drop table #t

/* -- replaced by srv.Wine_MatchWithSearchDB call below
...
update dbo.SYN_t_WAName set Wine_VinN_ID = r.Wine_VinN_ID
from r
	join dbo.SYN_t_WAName on r.idN = WAName.idN
...
-- rematch with newly created wines
exec srv.Wine_MatchWithSearchDB;
