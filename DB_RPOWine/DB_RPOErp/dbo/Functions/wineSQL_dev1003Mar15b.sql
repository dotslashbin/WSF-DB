CREATE  function [dbo].wineSQL_dev1003Mar15b ( 
	 @Select varChar(1000)
	,@maxRows int = 100
	,@Where varchar(max) = ''
	,@GroupBy varchar(max) = ''
...
if isNull(@whN,0) <> 0 begin
	if 0<>charIndex(',cellarInfo,', @Select2) begin
		...
		set @sm = 'when bottleLocations like ''%[^
 ]%'' then case when userComments like ''%[^
 ]%''  then bottleLocations + ''['' + userComments + '']'' else bottleLocations end else case when userComments like ''%[^
 ]%''  then ''['' + userComments + '']'' else '''' end end '
...
declare @s nvarchar(max) =  dbo.wineSQL_dev1003Mar15b(
	...
	--'Select wineN, vintage, fullWineName, myIconN, ratingShow, maturity, priceShow, valuation, bottleCount, notYetCellaredBottleCount, toBeDeliveredBottleCount,mostRecentDeliveryDate'     --@select
	'Select wineN, vintage, cellarInfo, myIconN, fullWineName'     --@select
	,100		--@maxRows,
...
update whToWine set userComments = 'Eurocave'               where whN=20 and wineN=69618
select * from tasting where tastingN = 5164404
select rating,ratingPlus,ratingHi,ratingShow,ratingQ,hasRating from tasting where tastingN = 5164404
