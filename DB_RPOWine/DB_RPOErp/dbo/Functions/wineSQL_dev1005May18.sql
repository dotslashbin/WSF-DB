CREATE  function [dbo].wineSQL_dev1005May18 ( 
	 @Select varChar(1000)
	,@maxRows int = 100
	,@Where varchar(max) = ''
	,@GroupBy varchar(max) = ''
...
-------------------------------------------------------------------------------------
if @priceGN > 0 begin
	--1005May06     if @GroupBy like '%[^ ]%' begin
	--1005May06     	if 0<>charIndex(',forSaleIconN,', @Select2) begin set @ss = replace (@ss, ',forSaleIconN,',',case when max(mostRecentPriceLo) > 0 then ' + @iconForSaleNow + ' else 0 end forSaleIconN,') end
	--1005May06     end
	--1005May06     else begin
...
-------------------------------------------------------------------------------------
--1005May06     if @groupBy like '%[^ ]%' begin
--1005May06     	if 0<>charIndex(',pubIconN,', @Select2) begin set @ss = replace(@ss, ',pubIconN,',',max(isnull(f.iconN, -1)) pubIconN,'); select @te=1, @tf=1 end
--1005May06     end
--1005May06     else begin			
...
if isNull(@whN,0) <> 0 begin
 ...
 ['' + userComments + '']'' end'
		...
		--1005May06     if @GroupBy like '%[^ ]%' set @sm = ' max(' + @sm + ')'
...
--1005May06     if @GroupBy like '%[^ ]%' begin
--1005May06     	Set @GroupBy = replace(@GroupBy, ' FullWineName', ' b.producerShow, b.labelName')
--1005May06     	Set @GroupBy = replace(@GroupBy, '(FullWineName', '(b.producerShow, b.labelName')
--1005May06     	Set @GroupBy = replace(@GroupBy, ' producerShow', ' producerShow')
--1005May06     	Set @GroupBy = replace(@GroupBy, '(producerShow', '(producerShow')
--1005May06     end
...
if @Errors is not null begin
	...
	if @where like '%[^ ]%'              set @R += '
	' + @where
	--1005May06     if @GroupBy like '%[^ ]%' set @R += '
	--1005May06     ' + @GroupBy
/*-----------------------------
...
set @s = dbo.wineSQL_dev1005May18(
