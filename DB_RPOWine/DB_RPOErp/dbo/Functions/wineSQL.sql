...
if 0<>charIndex(',activeVinn,', @Select2) begin set @ss = replace(@ss, ',activeVinn,',',a.activeVinn,') end
...
if 0<>charIndex(',fullWineNamePlus,', @Select2) begin set @ss = replace(@ss, ',fullWineNamePlus,',',b.matchName fullWineNamePlus,') end
...
if @priceGN > 0 begin
	if @priceGN=@priceGNAllRetailers
		begin
			if 0=isnull(@IncludeAuctions ,0)
				begin
					if 0<>charIndex(',forSaleIconN,', @Select2) begin set @ss = replace (@ss, ',forSaleIconN,',',case when a.isCurrentlyForSale=1 then ' + @iconForSaleNow + ' else 0 end forSaleIconN,') end
				...
				begin
					if 0<>charIndex(',forSaleIconN,', @Select2) begin set @ss = replace (@ss, ',forSaleIconN,',',case when a.isCurrentlyForSale=1  then ' + @iconForSaleNow + ' else 0 end forSaleIconN,') end
					if 0<>charIndex(',priceLo,', @Select2) begin set @ss = replace(@ss, ',priceLo,',',isNull(a.mostRecentAuctionPriceLoStd, a.estimatedCostLo) priceLo,') end
					if 0<>charIndex(',priceHi,', @Select2) begin set @ss = replace(@ss, ',priceHi,',',isNull(a.mostRecentAuctionPriceHiStd, a.estimatedCostHi) priceHi,') end
					if 0<>charIndex(',priceCnt,', @Select2) begin set @ss = replace(@ss, ',priceCnt,',',isNull(a.priceCnt,0) priceCnt,') end
					if 0<>charIndex(',priceShow,', @Select2) begin set @ss = replace(@ss, ',priceShow,',',dbo.formatPrice(a.mostRecentAuctionPriceLoStd,a.estimatedCostLo, a.mostRecentAuctionPriceHiStd, a.estimatedCostHi) priceShow,') end
					if 0<>charIndex(',priceShowRelease,', @Select2) begin set @ss = replace(@ss, ',priceShowRelease,',',dbo.formatPriceRelease(a.mostRecentAuctionPriceLoStd, a.mostRecentAuctionPriceHiStd,a.estimatedCostLo) priceShowRelease,') end
				end
		end
	else
		begin
...
*/
		...
		set @sm = ',dbo.showCellarInfo1(bottleLocations ,userComments) cellarInfo,'
...
declare @s nvarchar(max) =  dbo.wineSQL_dev1005May06(
	...
	,''--'Group by country'		--@GroupBy
j	,''--,'order by  fullWineNamePlus asc, Vintage desc'	--@OrderBy
	,9		--@PubGN
	,0--1		--@MustHaveReviewInThisPubG,
	,18220	--@PriceGN,
	,0		--@MustBeCurrentlyForSale,
...
declare @s nvarchar(max) =  dbo.wineSQL(
--	'Select ratingShow,wineN, vintage, fullWineName, fullWineNamePlus, colorClass, Site , Locale,Location,Region ,Country, variety, dryness,  winetype,ProducerShow,hasUserComplaint'			--@Select2
	'Select fullWineNamePlus'			--@Select2
	,100		--@maxRows,
	,'where contains ( encodedKeywords, ''  "petrus*"   '')  '				--@where
	,''		--'Group by ratingRange'		--@GroupBy
	,''		--'Order by cnt desc'				--@OrderBy
...
declare @s nvarchar(max) =  dbo.wineSQLdev(
	'Select wineN, vintage, cellarInfo, myIconN, fullWineNamePlus'     --@select
	,100		--@maxRows,
	--,'where contains ( encodedKeywords, ''  "pavie*" and "2003*"'')  '				--@where
	,'where isOfInterestX = 1  '				--@where
...
declare @s nvarchar(max) =  dbo.wineSQL(
	'Select fullWineNamePlus,cellarInfo'			--@Select2
	,100		--@maxRows,
	--,'where contains ( encodedKeywords, ''  "petrus*"   '')  '				--@where
	,'where isOfInterestX=1'     				--@where
	,''		--'Group by ratingRange'		--@GroupBy
...
declare @s nvarchar(max) =  dbo.wineSQL (
	'Select fullWineNamePlus,cellarInfo,rating,forsaleiconN,priceLo'			--@Select2
	,100		--@maxRows,
	,'where contains ( encodedKeywords, ''  "petrus*"   '')  '				--@where
	,''		--'Group by ratingRange'		--@GroupBy
	,''		--'Order by cnt desc'				--@OrderBy
