/*
use erp
print dbo.getMatchName('Producer','cab','Rose','Cab bll', 'USA', null,null,null,null)
print dbo.getMatchName('Producer','cabo','Rose','Cab blend', 'USA', null,null,null,'Salem')
print dbo.getMatchName('Producer',null,'Rose','Cab blend', 'USA', null,null,null,'Salem')
*/
CREATE function [dbo].getMatchName (@producerShow nvarchar(100),@labelName nvarchar(150)='',@colorClass nvarchar(20),@variety nvarchar(100),@country nvarchar(100),@region nvarchar(100),@location nvarchar(100),@locale nvarchar(100),@site nvarchar(100))
returns varchar(max)
as begin
...
if @variety is not null
	begin
		if @variety like '%blend%'
			set @v=ltrim(rtrim(replace(@variety,' blend','')))
		else 
			set @v=@variety
		
		if (' '+isnull(@labelName,'')+' ') not like ('%[^0-z]'+@v+'[^0-z]%')
			set @sp=@v+', '
	end
	
if @colorClass like '%[0-z]%' set @sp+=@colorClass+', '
