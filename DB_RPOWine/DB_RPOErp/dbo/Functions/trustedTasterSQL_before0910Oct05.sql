...
-----------------------------------------------------------------------------------------------------
CREATE  function trustedTasterSQL_before0910Oct05(@whN int, @contains nvarchar(1000)=null, @mustAlreadyBeTrustedByMe bit = 0)
returns varchar(max)
begin
...
select * from d '
 ...
 if @whN > 0 begin
	...
	set @s = replace(@s, '@andNotMe@', 'and tasterN <> '+@sx)
	set @s = replace(@s, '@whereMe@', 'where whN = '+@sx)
end
else begin
	set @s = replace(@s, '@andNotMe@', '')
