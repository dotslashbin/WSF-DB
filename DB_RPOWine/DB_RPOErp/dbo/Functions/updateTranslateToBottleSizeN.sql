...
as begin
	...
	begin
		...
		begin
			select @bottleSizeN = bottleSizeN from bottleSize where name = @bottleSize
			if @bottleSizeN is null
				select @bottleSizeN = bottleSizeN from bottleSize where name = dbo.superTrim(@bottleSize)
		end
	return @bottleSizeN	
end
