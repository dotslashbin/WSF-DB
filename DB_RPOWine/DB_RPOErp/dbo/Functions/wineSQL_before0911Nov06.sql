 CREATE  function [dbo].[wineSQL_before0911Nov06] ( 
	 @Select varChar(1000)
	,@maxRows int = 100
	,@Where varchar(max) = ''
	,@GroupBy varchar(max) = ''
...
declare @pubGNWineAdvocate int = 18223, @pubGNWineAdvocates int = 18240
...
-------------------------------------------------------------------------------------
 if @doGetAllTastings = 0 begin
	...
	if 0=charIndex(',isProTasting,', @Select2) set @ss += 'e.isProTasting,'
...
exists (select * 
	from tasting z 
		...
		where 
			z.wineN = e.wineN
				and y.whN = ' + convert(varchar, @whN)+'
)
'
				if @doGetAllTastings = -1 set @wherex = 'not ' + @wherex
...
if @pubGN is not null and @MustHaveReviewInThisPubG <> 0 
	begin
		...
		--	begin
				set @wherex = 'isErpTasting=1'
				if @MustHaveReviewInThisPubG = -1 set @wherex = replace(@wherex, '=1', '=0')
		--	end
		--else
		--	begin
		--		set @wherex = 'exists(select * from tasting where wineN=e.wineN and isErpTasting=1)'
		--		if @MustHaveReviewInThisPubG = -1 set @wherex ='not ' + @wherex
		--	end
			
		if @pubGN <> @pubGNWineAdvocate set @wherex = replace(@wherex, 'isErpTasting','isProTasting')
		set @where += case when @where like '%where%' then ' and ' else 'where ' end + @wherex
...
@where=where contains ( encodedKeywords, '"pavie*" and "2003*"')  and ("hasUserComplaint"='0' or ("hasUserComplaint"='1' and "isApproved"='1') or "hasUserComplaint" is null)
@GroupBy=’’
@OrderBy=’ order by  fullWineName asc, Vintage desc’
@PubGN=9
@PriceGN=18220
@whN=20
@doFlagMyBottlesInMyWines=True
@IncludeAuctions=True
@MustHaveReviewInThisPubG=0
@MustBeCurrentlyForSale=0
@mustBeTrustedTaster=0
 
 
declare @s nvarchar(max) =  dbo.wineSQL_after0911Nov04(
	'Select ratingMinor, TastingN,wineN, vintage, fullWineName, colorClass, publication, pubiconN,  forSaleIconN, priceLo, estimatedCostLo, priceShow, priceShowRelease, ratingShow, maturity,MyIconN,MyBottleCount,ProducerURL, Site , Locale,Location,Region ,Country, variety, dryness,  winetype,ProducerShow,rating,hasUserComplaint'			--@Select2
	,100		--@maxRows,
	,'where contains ( encodedKeywords, ''  "petrus*" '')  and (hasUserComplaint=0 or (hasUserComplaint=1 and isApproved=1) or hasUserComplaint is null)'				--@where
	,''		--@GroupBy
	,'order by  fullWineName asc, Vintage desc'	--@OrderBy
	,18240		--@PubGN
	,0		--@MustHaveReviewInThisPubG,
