/*
use erp
finishImport_dev1003Mar21 20
finishImport 20
*/
CREATE proc finishImport_before1003Mar22(@whN int) as
--alter proc finishImport_dev1003Mar21 (@whN int) as
begin
--declare @whN int = 20
set noCount on
declare  @importStatus int = 0
begin try
	declare	@myWineName varchar(200), @Supplier varchar(max), @Quantity varchar(max), @BottleSize	varchar(max), @PurchaseDate datetime, @deliveryDate datetime, @Price varchar(max), @Location varchar(max), @wineNFromMatch int, @myVintage varchar(max)
	...
	declare 
		@mp cursor
		, @sql nvarchar(max) =  N'Set @mpOut = cursor for Select MyWineName,Supplier, Quantity, BottleSize, PurchaseDate, DeliveryDate, Price, Location, wineN, myVintage from transfer..userWines' + convert(nvarchar, @whN) + N'; open @mpOut';
	...
	while 1=1
		begin
			fetch next from @mp into @myWineName, @Supplier, @Quantity, @BottleSize, @PurchaseDate, @DeliveryDate, @Price, @Location, @wineNFromMatch, @myVintage
											...
											update b set 
												...
												where
													...
													and
														(
															b.isOld <> 0 
															or b.dateLo <> a.dateLo or not((a.dateLo is null and b.dateLo is not null)  and (a.dateLo is not null and b.dateLo is  null))
															or b.dateHi <> a.dateHi or not((a.dateHi is null and b.dateHi is not null)  and (a.dateHi is not null and b.dateHi is  null))
														)
										end									
