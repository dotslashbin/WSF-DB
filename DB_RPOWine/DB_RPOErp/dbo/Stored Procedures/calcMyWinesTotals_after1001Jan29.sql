CREATE proc [dbo].calcMyWinesTotals_after1001Jan29 (@whN int, @wineN int)
as begin
--declare @whN int = 20, @wineN int = null
/*
ToDo
	Decide whether to add Consumed an integer
	Correctly calculate stages from on order to consumed

oofrr 
calcMyWinesTotals 20, null
[calcMyWinesTotals_after1001Jan29] 20, null
...
select wineN, bottleLocations, toBeDeliveredBottleCount, toBeDeliveredX, notYetCellaredBottleCount,notYetCellaredX,bottleCount,tastingCount,wantToSellBottleCount,wantToBuyBottleCount,buyerCount,sellerCount,purchaseCount 
	from  whtowine where whn=20 and wineN=88041
*/ 
set noCount on
...
,o as (select wineN, count(*) calcCellaredBottleCount 
		from location oa
			join purchase ob
