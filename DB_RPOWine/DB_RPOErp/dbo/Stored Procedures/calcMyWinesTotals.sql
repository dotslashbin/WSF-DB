CREATE proc [dbo].calcMyWinesTotals(@whN int, @wineN int)
--ALTER proc [dbo].calcMyWinesTotals_dev1003Mar18 (@whN int, @wineN int)
as begin
--declare @whN int = 20, @wineN int = 29481	---29478
set noCount on
...
,o as (select wineN
			, isNull(sum(isNull(bottleCountAvailable, 0)), 0) calcCellaredBottleCount 
...
,b as (select *
			--dwf--, case when deliveryDate is null or deliveryDate <= '1900' or deliveryDate > @today then 1 else 0 end isOnOrder
			, case when (deliveryDate is null or deliveryDate < '1901' or deliveryDate > @today) and purchaseDate >= '1901' then 1 else 0 end isOnOrder
		from purchase wh
...
,d as (select da.whN, da.wineN
		...
		, case when (c.remainingBottleCount + c.toBeDeliveredBottleCount) > 0 then (c.remainingBottleCount + c.toBeDeliveredBottleCount) when da.bottleCount < 0 then da.bottleCount else 0 end effectiveBottleCount
