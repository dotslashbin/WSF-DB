...
CREATE proc [dbo].[updateWhToWineRecord](@whN int, @wineN int, @fieldname varchar(50), @param int, @comm varchar(250))
...
set noCount on;
      ...
      -- if @fieldname ='userComments'
       ...
       if    @fieldname ='hasBottles' 
         update whToWine set hasBottles=@param where wineN=@wineN and whn=@whn 
       ...
       if    @fieldname ='toBeDelivered' 
         update whToWine set toBeDeliveredBottleCount=@param where wineN=@wineN and whn=@whn 
         
        if    @fieldname ='notYetCellared' 
         update whToWine set notYetCellaredBottleCount=@param where wineN=@wineN and whn=@whn 
