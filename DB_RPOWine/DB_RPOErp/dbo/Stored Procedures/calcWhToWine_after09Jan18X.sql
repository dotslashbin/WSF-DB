CREATE proc [dbo].calcWhToWine_after09Jan18X (@whN int, @wineN int, @fullRecalculateFromMyWinesTables bit = 0)
as begin
set noCount on;
if @fullRecalculateFromMyWinesTables <> 0
            exec dbo.[calcMyWinesTotals_after10Jan18] @whN, @wineN
else 
            begin
                        if not exists (select * from whToWine where whN = @whN and (@wineN is null or wineN = @wineN))
