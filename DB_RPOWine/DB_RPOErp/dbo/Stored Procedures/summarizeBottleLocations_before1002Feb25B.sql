CREATE proc summarizeBottleLocations_before1002Feb25B (@whN int, @wineN int) as
...
-----------------------------------------------------------------------------------
/*
summarizeBottleLocations_after1002Feb25 20,null
...
, k2 as (select wineN, replace(replace(replace(k.bottleLocations, ',,,,,', ','), ',,,', ','), ',,', ',') bottleLocations
	from k)
,m as (select whN, wineN, bottleLocations from whToWine where whN=@whN)
merge m as ma
	using (select * from k2) as mb
		on  ma.whN = @whn and ma.wineN = mb.wineN
when matched then
	update set ma.bottleLocations = mb.bottleLocations
when not matched by source then
	update set ma.bottleLocations = null;
