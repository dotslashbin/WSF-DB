...
CREATE proc [dbo].[setMyWine] @whN int, @wineN int, @on bit = null, @bottleCntNeeded bit = 1, @tastingCntNeeded bit = 1
as begin
...
if  @bottleCntNeeded  = 1 
	begin
		if exists (select top 1 *  from purchase where whN = @whN and wineN = @wineN)
			set @hasPurchases = 1
		else
			set @hasPurchases = 0
 
		select @bottleCount = isNull(sum(bottleCount), 0)
			from purchase where whN = @whN and wineN = @wineN and bottleCount > 0;
			
		select @bottleCount +=  isNull(sum(bottleCountHere), 0)
			from purchase a
				join bottleLocation b
