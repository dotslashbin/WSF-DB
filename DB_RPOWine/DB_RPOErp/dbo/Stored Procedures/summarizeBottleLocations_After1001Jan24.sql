create proc summarizeBottleLocations_After1001Jan24 (@whN int, @wineN int) as
begin
...
/*
use erp
summarizeBottleLocationsX 20, null
...
,b as (select wineN, litres, max(nameInSummary) nameInSummary
		...
		, sum (case when deliveryDate is null or deliveryDate <= '1900' or deliveryDate > purchaseDate then bottleCount else 0 end) notYetDeliveredBottleCount
		, sum (case when deliveryDate is null or deliveryDate <= '1900' or deliveryDate > purchaseDate then 0 else isNull(bottlesRemaining, bottleCount) end) remainingBottleCount
	from a
	group by wineN, litres)

,g as (select top 199999 b.wineN, nameInSummary, litres, e.bottleCountHere bottleCount, f.path, f.iiPath, f.depth, f.ii
	from a
		join wine b
			on a.wineN = b.wineN
		join location e
