 CREATE proc summarizeBottleLocations_after1001Jan30 (@whN int, @wineN int) as
begin
...
/*
restoreUser 20, 'Bug D219'
restoreUser 20, '1001Jan28 before zapping for consumed'
restoreUser 20,'Big'
 
use erp
summarizeBottleLocations_after1001Jan30 20, null
 
select b.purchaseN, bottleLocations, wineN, deliveryDate,bottleCount, bottleCountBought,LocationN,parentLocationN,currentBottleCount,maxBottleCount,bottleCountHere,bottleCountAvailable,locationCapacity
	from location a
		right join purchase b
			on a.whN=b.whN and a.purchaseN=b.purchaseN
		right join (select whN, wineN wineN2, bottleLocations from whToWine) c
			on c.whN = b.whN and c.wineN2 = b.wineN	
	where b.whN=20
...
select * from purchase where purchaseN = 3833
...
with
o as (select op.wineN, oa.* 
		from location oa
			join purchase op
				on op.purchaseN=oa.purchaseN and op.whN=oa.whN
		where op.whN = @whN and (@wineN is null  or op.wineN = @wineN) and isBottle = 1)
...
,g as (select top 199999 b.*, litres
		...
		, o.bottleCountAvailable bottleCountAvailable
	...
	from a
		...
		join @TT f
			...
			on f.locationN = o.parentLocationN
	--where e.whN = @whN and (@wineN is null  or a.wineN = @wineN)
	where a.whN = @whN and (@wineN is null  or a.wineN = @wineN)
