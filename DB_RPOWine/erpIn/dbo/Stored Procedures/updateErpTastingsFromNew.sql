CREATE proc updateErpTastingsFromNew as begin
/*
use erpin
buildMerge 'tasting', 'tastingN rowversion                 hasRating isErpTasting isProTasting parkerZralyLevel sourceIconN ratingShow updateDate wineNameN vinn'
	(trailing fields are set by triggers on the tasting table)
 
alter view vTasting as select * from erp..tasting where dataSourceN is not null and dataIdN is not null
alter view vTasting as select * from erp..tastingSnap where dataSourceN is not null and dataIdN is not nullh
oodef vTasting
	select count(*)from vtasting
	select count(*)from erp..tastingSnap
 
	select count(*)from vtastingNew
	select count(*)from erpin..tastingNew
 
 
 
alter view vTastingNew as select * from tastingNew where dataSourceN is not null and dataIdN is not null
alter view vTastingNew as select * from erpIn..tastingNew where dataSourceN is not null and dataIdN is not null
...
------------------------------------------------------------------------------------------------------------------------------
update tastingNew set isPrivate=0 where isPrivate is null
update tastingNew set isAnnonymous=0 where isAnnonymous is null
...
exec erp..updateSpeed

print 'Tastings updated: '+convert(char, @@rowCount)
