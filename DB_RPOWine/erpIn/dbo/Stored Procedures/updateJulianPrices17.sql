--use erpin
create proc updateJulianPrices17 as begin
...
update a 
	set 
		   isWineNameVS = 1
		, VSFirstDate = case when VSFirstDate is null then @Date else VSFirstDate end
		, VSActiveDate = case when isPriorWineNameVS = 0 then @Date else VSActiveDate end
		, VSLastDate = @Date
	where ii = 1;
...
update a
	set
		  isNewAdtiveVinnNotYetApproved =
			case when activeVinn is null then 0 else 1 end
		, activeVinn = vinn
	from wineName a
		join idUse b
			on a.wineNameN = b.wineNameN
		where b.isWineNameVS = 1
			and a.activeVinn <> b.vinn or not (a.activeVinn is null and vinn is null);
...
/*update a
	...
	from wineName a
		...
		join idUse b
			on a.wineNameN = b.wineNameN
		where b.isWineNameVS = 0
			and a.activeVinn is not null
*/
