--use erpin
CREATE proc updateJulianPrices27 as begin
--updateJulianPrices26
set noCount on
...
update a set a.producer2 = b.producer, a.producerShow2 = b.producerShow
	from vAlertOK a
		join (select vinn, producer, producerShow, row_number() over(partition by vinn order by producer)jj from erpWine)b
...
with
b as (select *, row_number() over(partition by wineN order by fixedId)jj from erpWine)
...
,g as (select wid,vintage, max(wineN) wineN, max(vinn) vinn
	...
	having count(*) = 1)
,h as (select *, row_number() over(partition by wineN order by fixedId)jj from erpWine)
insert into nameYearRaw(phase, wineN, vinn, wid, producer, producerShow, labelName,vintage,country, region, location, variety, colorClass, dryness, wineType,dateCreated, dateUpdated)
