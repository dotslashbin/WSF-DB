--use erpin
CREATE proc updateNamesFromErpAndWs2 as begin
...
insert into erp..wh (isRetailer,shortName,fullName,displayName,sortName,address,city,state,postalCode,country,comments ,phone,fax,email)
	select 1
...
with a as (select z.* from vForSale z left join erp..currencyConversion y on z.currency = y.alertcurrency 
...
with a as (select z.* from vForSale z left join erp..bottleSizes y on z.BottleSize = y.BottleSize 
...
with a as (select z.* from vForSale z left join erp..retailers y on z.retailerCode = y.retailerCode 
...
update a set a.warnings = isNull(a.warnings + ';     ', '') + '[W02]  WineAlert Vinn not in eRP Database', vinn2=null
	from vAlertOK a
		left join (select distinct vinn from vErpWine) b
			on a.vinn2 = b.vinn
...
with 
a as (select distinct vinn from vErpWine where vinn is not null)
,b as (select * from vAlert where wid is not null and vinn2 is not null)
update b set b.errors = isNull(errors + ';  ', '') + '[E02]  Vinn not in vErpWine'
	from b
		left join a
			on a.vinn = b.vinn2
...
update a set a.producer2 = b.producer, a.producerShow2 = b.producerShow
	from vAlertOK a
		join (select vinn, producer, producerShow, row_number() over(partition by vinn order by producer)jj from vErpWine)b
...
with a as	(select distinct wineN from vErpWine)
update b set b.errors = isNull(errors + ';  ', '') + '[E04]  WineN not in vErpWine'
	from vForSale b
		left join a
			on a.wineN = b.wineN
...
with 
a as (select wineN, min(vinn)vinn from vErpWine group by wineN)
...
--baaaa>		make sure we have all the vinns already in use from vErpWine and vAlert 
...
--bbb>     add in erp wine where wineN can be uniquely deduced from vErpWine through wid=>vinn, vintage
...
merge allocateWidVinn as aa
	using	(select vinn from vErpWine group by vinn)bb
	on aa.vinn = bb.vinn
when matched then
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--bba>.    add in erpFields for vForSale items with wineN	
--bbb>.    add in erp wine where wineN can be deduced from vErpWine through wid=>vinn, vintage
...
with
b as (select *, row_number() over(partition by wineN order by fixedId)jj from vErpWine)
...
--     find case from vErpWine where there is unique vintage,wid
--     declare @date smallDateTime = getDate();
with 
...
,g as (select wid,vintage, max(wineN) wineN, max(vinn) vinn
	...
	having count(*) = 1)
,h as (select *, row_number() over(partition by wineN order by fixedId)jj from vErpWine)
insert into nameYearRaw(phase, wineN, vinn, wid, producer, producerShow, labelName,vintage,country, region, location, variety, colorClass, dryness, wineType,dateCreated, dateUpdated)
...
--select erp.dbo.convertSurname('foo, joe')
with
a as (select erp.dbo.convertSurname(producer) producerShow2,producerShow from wineName)
update a
