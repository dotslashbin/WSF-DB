CREATE proc updateWineName as begin
set nocount on;
/*
USE erpin
alter view vWineName as select * from erp13..wineName
create view vWine as select * from erp13..wine
...
------------------------------------------------------------------------------------------------------------------------------
/*
buildMergeWinename 'Winename','dateLo dateHi vintageLo vintageHi rowVersion wineCount isLive wineNameN'
 
select joinx, count(*) cnt from wineName group by joinx order by cnt desc
 
with a as (select joinx, count(*)cnt  from wineName group by joinx)
select * from wineName where joinx in (select joinX from a where cnt>1) order by joinx
 
with a as (select joinx, count(*)cnt  from vwineName group by joinx)
select * from vwineName where joinx in (select joinX from a where cnt>1) order by joinx
...
when not matched by source and a.dateHi is null then
	update set 
		dateHi=@date;
print 'merged: ' + convert(nvarchar, @@rowCount)
